...
class SalvaInfosPartida
{

    function dataPartidasLoop($puuid,  $fila )
    {
        ...
        while ($condicaoData) {
            ...
            if ($anosDeDiferenca == "0" && $mesesDeDiferenca == "0") {
                ...
                $this->consultaPartidas($datas, $puuid,  $fila);
                $condicaoData = false;
            }
        }
    }
    ...
    function consultaPartidas($datas,  $puuid , $fila)
    {
        ...
        foreach ($resultInvocador as $key => &$val) {
            ...
            foreach ($datas as &$valor) {
                ...
                $this->ConsultaSoloDuo($datasRequest, $val['puuid'], $token, $fila);
            }
        }
    }

    function ConsultaSoloDuo($datasRequest, $puuid, $token, $fila)
    {
        ...
        foreach ($datasRequest as $key => &$valor) {
            if ($key == 'primeiraDiaMes') {
                ...
                $this->SalvaDadosPartidas($partidas, $token, $puuid,  $fila);
            } else if ($key == 'fimMes') {
